@php
    $path = [
        'features' => 'Features',
        'how-it-works' => 'How It Works',
        'pricing' => 'Pricing',
        'faq' => 'FAQ',
    ];
@endphp
<nav class="font-serif px-16 py-4 flex justify-between items-center border-b-gray-200 border-b-2">
    <div class="text-blue-800 logo hover:scale-95 flex items-center justify-center font-bold gap-0.5">
        <a href="{{ route('welcome') }}" class="flex items-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="size-6">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
            <logo class="font-bold text-2xl cursor-pointer">DailyAssist</logo>
        </a>
    </div>
    <ul class="flex">
        @foreach ($path as $key => $value)
            <li>
                <a href="{{ route($key) }}"
                    class="hover:text-blue-900 hover:border-b-blue-800 hover:border-b-2 px-4 py-2">
                    {{ $value }}
                </a>
            </li>
        @endforeach
    </ul>
    <div>
        <button class="py-1.5 px-4 hover:bg-gray-200 rounded-full">Login</button>
        <button class="py-1.5 px-4 bg-blue-800 hover:bg-blue-700 hover:scale-95 text-white rounded-full">
            Get Started
        </button>
    </div>
</nav>
